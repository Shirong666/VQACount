...
class Fusion(nn.Module):
    ...
    def __init__(self, in_dim):
        super().__init__()
        # embed_dim = in_dim
        self.multihead_attn = nn.MultiheadAttention(in_dim, config.num_heads, batch_first=True)
    ...
    def forward(self, vision, question):
        ...
        # 1) used in attention class. 
        # after arraging the tensor, then fed into model
        if len(vision.size())>2:
            vision = vision.squeeze()
            question = question.squeeze()
            vision =vision.permute(0, 2, 1)
            question =question.permute(0, 2, 1)
            attn_output, attn_output_weights = self.multihead_attn(question, vision, vision)
            return attn_output.permute(0,2,1).unsqueeze(2)
    
        # 2) used in classifier class. Make tensor form 2D into[batch, source, embeding]
        # after arraging the tensor, then fed into model
        elif len(vision.size())==2:
...
class Classifier(nn.Sequential):
    def __init__(self, in_features, mid_features, count_features, out_features, drop=0.0):
        ...
        self.relu = nn.ReLU()
        self.fusion = Fusion(mid_features)
